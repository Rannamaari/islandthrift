...
export const usePosStore = defineStore('pos', () => {
    ...
    function printActiveSale(sale = activeSaleLookup.value ?? saleCompleteModal.value, format = 'thermal') {
        ...
        const shopName = receiptProfile.shop_name || receiptProfile.receipt_shop_name || sale.branch?.name || receiptProfile.name || 'Island Thrift';
        ...
        const thermalReceipt = `<header class="center"><div class="shop">${escape(shopName)}</div><div class="muted">${escape(gstLabel)}: ${escape(taxNumber)}</div>${showAddress && address ? `<div class="muted">${escape(address)}</div>` : ''}${showPhone && receiptProfile.phone ? `<div class="muted">${escape(receiptProfile.phone)}</div>` : ''}${header}</header>
            ...
            <div class="rule"></div><div class="payments">${payments}</div><div class="footer">${footer}</div>`;
        ...
        receipt.document.write(`<!doctype html><html><head><title>${isA4Invoice ? 'Tax Invoice' : 'Receipt'} ${escape(sale.sale_number)}</title><style>${styles}</style></head><body><div class="print-controls"><button type="button" onclick="window.print()">Print</button></div>${isA4Invoice ? a4Invoice : thermalReceipt}</body></html>`);
