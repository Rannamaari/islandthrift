...
<div class="grid gap-9 lg:grid-cols-2 lg:gap-14">
    ...
    <div class="lg:py-5"><p class="store-kicker">{{ $product->category?->name ?? 'Electronics' }}</p><h1 class="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{{ $product->name }}</h1>@if($product->short_description)<p class="mt-4 text-lg leading-8 text-slate-600">{{ $product->short_description }}</p>@endif
        <div class="mt-6 flex items-baseline gap-3"><strong class="text-3xl font-black">{{ $product->branchPrices->first()?->currency ?? 'MVR' }} {{ number_format($price, 2) }}</strong>@if($price < $regular)<del class="text-lg text-slate-400">{{ number_format($regular, 2) }}</del>@endif</div>
        ...
        @if($product->description)<div class="mt-9 border-t border-slate-200 pt-7"><h2 class="text-lg font-black">Product details</h2><div class="mt-3 whitespace-pre-line text-sm leading-7 text-slate-600">{{ $product->description }}</div></div>@endif
    </div>
</div></section>
